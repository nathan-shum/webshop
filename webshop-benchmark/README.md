...
## 🐳 Docker Quickstart (Text Env)

### Prerequisites: Download Data Files

The WebShop environment requires product data files (1000 products, ~50MB total):

```bash
mkdir -p data && cd data
pip install gdown
gdown 1EgHdxQ_YxqIQlvvq5iKlCrkEKR6-j0Ib  # items_shuffle_1000.json
gdown 1IduG0xl544V_A_jv3tHXC0kyFi7PnyBu  # items_ins_v2_1000.json
gdown 14Kb5SPBk_jfdLZ_CDBNitW98QLDlKR5O  # items_human_ins.json
cd ..
```
...
make up          # build & start
make health      # -> {"ok": true}
make reset       # start an episode (goal_index 0)
make step        # take an action, see obs/reward
make score       # cumulative reward
...
* `GET  /health` → `{ "ok": true }`
* `POST /reset`  body: `{ "goal_index"?: number }` → `{ "observation": "...", "info": {...}, "available_actions": [...] }`
* `POST /step`   body: `{ "action": "search[...]" | "click[...]" }` → `{ "observation": "...", "reward": 0.0, "done": false, "info": {...}, "available_actions": [...] }`
* `GET  /score`  → `{ "total_reward": 0.0 }`

This container runs the **text** environment only; it's meant to be driven by an external agent via HTTP. The container uses lightweight BM25 search (no PyTorch/transformers) for minimal dependencies.
